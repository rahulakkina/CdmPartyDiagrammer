var AdfDvtLayoutSupport = {
};

AdfDvtLayoutSupport.getMaxNodeBounds = function (layoutContext, excludeLabelBounds) {
...
AdfDvtLayoutSupport.centerNodeAndLabel = function (node, x, y) {
  AdfDvtLayoutSupport.centerNode(node, x, y);
  AdfDvtLayoutSupport.positionNodeLabel(node);
};

AdfDvtLayoutSupport.centerNode = function (node, x, y) {
  var bounds = node.getContentBounds();
  node.setPosition(new DvtDiagramPoint(x - bounds.x - bounds.w * .5, y - bounds.y - bounds.h * .5));
};

AdfDvtLayoutSupport.positionNodeLabels = function (layoutContext) {
...
AdfDvtLayoutSupport.positionNodeLabel = function (node) {
...
AdfDvtLayoutSupport.getNodeComparator = function (attribute) {
...
AdfDvtLayoutSupport.layoutLinks = function (layoutContext) {
  for (var li = 0;li < layoutContext.getLinkCount();li++) {
    var link = layoutContext.getLinkByIndex(li);
    var endpoints = AdfDvtLayoutSupport.getEndpoints(layoutContext, link);
    ...
    /*
    //test curved links (allow "directSmooth" because jspx 
    //already supports it)
    if (type === "curved" || type === "directSmooth") {
      var mx = startX + .5 * (endX - startX);
      var my = startY + .5 * (endY - startY);
      //link.setPoints(["M", startX, startY, 
      //                "L", endX, endY]);
      //link.setPoints(["M", startX, startY, 
      //                "Q", mx+200, my+200, endX, endY]);
      //var angle = Math.PI / 3;
      //var mat = new DvtMatrix();
      //mat.rotate(-angle);
      //var newPoint = mat.transformPoint(new DvtDiagramPoint(mx - startX, 
      //                                               my - startY));
      //link.setPoints(["M", startX, startY, 
      //                "Q", startX + newPoint.x, startY + newPoint.y, 
      //                     endX, endY]);
      var angle = Math.PI / 4;
      var length = Math.sqrt((endX - startX) * (endX - startX) + (endY - startY) * (endY - startY));
      var halfLength = .5 * length;
      var hyp = halfLength / Math.cos(angle);
      var lenRatio = hyp / length;
      var x2 = startX + lenRatio * (endX - startX);
      var y2 = startY + lenRatio * (endY - startY);
      var mat = new DvtMatrix();
      mat.rotate( - angle);
      var newPoint = mat.transformPoint(new DvtDiagramPoint(x2 - startX, y2 - startY));
      link.setPoints(["M", startX, startY, "Q", startX + newPoint.x, startY + newPoint.y, endX, endY]);
    }
    else {
    ...
    */
    ...
    if (labelBounds) {
      var labelX = startX + .5 * (endX - startX - labelBounds.w);
...
AdfDvtLayoutSupport.getEndpoints = function(layoutContext, link) {
  ...
  if (bLinkToBounds) {
    ...
    var startP = AdfDvtLayoutSupport._intersectRect(b1, startX, startY, endX, endY, link.getStartConnectorOffset());
    var endP = AdfDvtLayoutSupport._intersectRect(b2, endX, endY, startX, startY, link.getEndConnectorOffset());
    startX = startP.x;
    startY = startP.y;
    endX = endP.x;
    endY = endP.y;
...
AdfDvtLayoutSupport._intersectRect = function (rect, startX, startY, endX, endY, connOffset) {
  var SIDE_TOP = 0;
  var SIDE_RIGHT = 1;
  var SIDE_BOTTOM = 2;
  var SIDE_LEFT = 3;
